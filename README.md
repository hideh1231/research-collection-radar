...
```text
python -m radar --open-only --ingest-html apa-cfp=apa.html
python -m radar --open-only --ingest-html sciencedirect-cfp=elsevier.html
python -m radar --open-only --ingest-html royal-society-themes=rs-ai.html
```

`--open-only` skips closed listing rows. ScienceDirect also drops titles and journals that do not match psychology, HCI, neuroscience, robotics, or HRI. Listing ingest does not fetch other sources or rewrite their crawl status. Browser automation is out of scope. Decision record: [`docs/decisions/0004-listing-snapshot-coverage.md`](docs/decisions/0004-listing-snapshot-coverage.md).
