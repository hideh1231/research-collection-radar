...
2026-09-12 更新: T&F の定期取得は、日次の公式 `special_issues` API（`taylor-francis-special-issues`）へ移行した。全ページの取得を確認できたため、週次の Author Services ブラウザ取込を終了した。`tandf-cfp` の HTML 取込は手動指定用に残す。現在の取得範囲は [source inventory](../source-inventory.md) を参照する。
