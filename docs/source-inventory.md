...
- APA, Elsevier ScienceDirect, APS, Science Robotics, SAGE, PNAS, PNAS Nexus, JOSA A, and watched Wiley journals use the existing weekly rendered-listing ingestion or explicitly supplied HTML snapshots. They remain disabled for the ordinary HTTP crawl. A browser challenge is recorded as a failure; it is not bypassed.
- Taylor & Francis uses the enabled daily `taylor-francis-special-issues` API source. Its redundant Author Services browser job was retired after a GitHub run captured an empty loading shell. The legacy `tandf-cfp` parser remains available for explicitly supplied snapshots, and its readiness check now requires parseable call cards.
